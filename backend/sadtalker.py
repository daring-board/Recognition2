...
class SadTalker():
    ...
    def execute(self, source_image, driven_audio, preprocess='crop', still_mode=False, use_enhancer=False, result_dir='./results/'):
        function_name = sys._getframe().f_code.co_name
        class_name = self.__class__.__name__
        print('Start: {}.{}'.format(function_name, class_name))
        ...
        if os.path.isfile(driven_audio):
            ...
            if '.mp3' in audio_path:
                raise AttributeError("error audio: allow only wav")
            else:
...
if __name__ == '__main__':
    ...
    st = SadTalker(checkpoint_dir, config_path)

    img_path = './avater/no_back_crop1.png'
    audio_path = './tts_out/bot.wav'
    movies_path = './movies'
    output_path = st.execute(
        img_path, audio_path,
        preprocess='resize',
        still_mode=False,
        use_enhancer=False,
